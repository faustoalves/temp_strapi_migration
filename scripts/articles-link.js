...
const init = async () => {
  ...
  let adjustedArticlesLinks = await Promise.all(
    articlesLinkList.map(async (link, index) => {
      ...
      console.log(newLink);
      // delete newLink.id;
      delete newLink.component;
      delete newLink.locale;
      delete newLink.createdAt;
      delete newLink.updatedAt;
      delete newLink.publishedAt;
      delete newLink.createdBy;
      delete newLink.updatedBy;
      delete newLink.localizations;
      return newLink;
    })
  );
  adjustedArticlesLinks = adjustedArticlesLinks.filter(
    (link) => link.components.length > 0
  );
  Promise.all(
    adjustedArticlesLinks.map(async (link, index) => {
      // console.log(JSON.stringify(link));
      try {
        setTimeout(() => {}, 200 * index, "initial Tokens");
        await ArticlesController.createArticleLink(link);
      } catch {
        console.log(link);
      }
    })
...
const parseComponents = async (components, title) => {
  let newComponents = await Promise.all(
    components.map(async (component, index) => {
      try {
        return await componentsType[component.__component](component, title);
      } catch (error) {
  ...
  let retComponents = [];
  await Promise.all(
    newComponents.map(async (component, index) => {
      if (Array.isArray(component)) {
        retComponents = [...retComponents, ...component];
      } else {
