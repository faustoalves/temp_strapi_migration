...
const fs = require("fs");
const { getOriginToken } = require("./repositories/TokenRepositories");
...
const init = async () => {
  ...
  let gallery = await GalleriesController.getGalleryByName(
    "Blumenschmuck Gallery"
  );
  console.log(gallery.images);
  // await PeopleController.getAtualTeam();
  ...
  // let articlesList = await ArticlesControllers.getArticlesList();
  // let allArticles = await Promise.all(
  //   articlesList.map(async (article, index) => {
  //     setTimeout(() => {}, index * 100, "initial Tokens");
  // let newArticle = await ArticlesController.getActualArticleById(114);
  // console.log(newArticle.components.length);
  // newArticle.components = await parseComponents(newArticle.components);
  // console.log(newArticle.components.length);
  // console.log(newArticle.components);
  // //   })
  // // );
